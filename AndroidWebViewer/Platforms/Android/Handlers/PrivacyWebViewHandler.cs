...
public class PrivacyWebViewHandler : WebViewHandler
{
    ...
    public static PrivacyWebChromeClient? CurrentChromeClient { get; private set; }
    ...
    protected override void ConnectHandler(WebView platformView)
    {
        ...
        if (BlockHosts.Count == 0)
            try
            {
                using var stream = FileSystem.OpenAppPackageFileAsync("blocklist.txt").Result;
        ...
        try
        {
            CookieManager.Instance.SetAcceptThirdPartyCookies(platformView, Allow3pCookies);
        }
        catch
        {
        }
        ...
        var activity = Platform.CurrentActivity ??
                       Application.Current?.Handler?.MauiContext?.Services.GetService(typeof(Activity)) as Activity;
        var chrome = new PrivacyWebChromeClient(activity!);
    ...
    private class HardenedClient : WebViewClient
    {
        ...
        private bool LooksLikeAdOrTracker(string path)
        {
            ...
            if (p.Contains("/ads/") || p.Contains("/adserver/") || p.Contains("/advert") || p.Contains("/banner"))
                return true;
            if (p.Contains("gpt.js") || p.Contains("adscript") || p.Contains("/analytics") ||
                p.Contains("analytics.js"))
                return true;
